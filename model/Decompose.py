...
from model.Other import FourierLayer
...
class Decompose(nn.Module):
    def __init__(self,
        ...
        self.dct_layer=dct_channel_block(d_model)
        #self.asb = Adaptive_Spectral_Block(enc_in)
    ...
    def forward(self, inp):
        ...
        main = self.LD(inp)
        residual, _ = self.seasonality_model(inp - main)  # [batch_size, win_size, channels]
        #residual = inp - main

        trend = main
        res_1 = residual
        #for i in range(self.n_layers):
        #    res_1 = self.dct_layer(res_1.permute(0, 2, 1)).permute(0, 2, 1)
        for i in range(self.n_layers):
            res_1 = self.auto_attn_blocks[i](res_1)
        for i in range(self.n_layers):
            trend = self.dct_layer(trend.permute(0, 2, 1)).permute(0, 2, 1)
        #for i in range(self.n_layers):
        #    trend = self.channel_attn_blocks[i](trend)
...
class LD(nn.Module):
    def __init__(self, kernel_size=25):
        ...
        self.conv = nn.Conv1d(1, 1, kernel_size=kernel_size, stride=1, padding='same',
    ...
    def forward(self, inp):
        # Permute the input tensor to match the expected shape for 1D convolution (B, N, T) N是channels, T是win_size
